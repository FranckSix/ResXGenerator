...
public readonly record struct FileOptions
{
    ...
    )
    {
        ...
        LocalNamespace = ResolveNamespace(options, globalOptions, resxFilePath, detectedNamespace);
    ...
    private static string ResolveNamespace(AnalyzerConfigOptions options, GlobalOptions globalOptions, string resxFilePath, string detectedNamespace)
    {
        if (options.TryGetValue("build_metadata.EmbeddedResource.TargetPath", out var targetPath) && targetPath is { Length: > 0 })
            return Utilities.GetLocalNamespace(resxFilePath, targetPath, globalOptions.ProjectFullPath, globalOptions.ProjectName, globalOptions.RootNamespace);

        return string.IsNullOrEmpty(detectedNamespace)
