...
public static class Utilities
{
    ...
    private static bool IsValidLanguageName(string? languageName)
    {
        try
        {
            ...
            if (languageName != null && languageName.StartsWith("qps-", StringComparison.Ordinal))
            {
                return true;
            }
    ...
    public static string SanitizeNamespace(string ns, bool sanitizeFirstChar = true)
    {
        ...
        return char.IsDigit(sanitizedNs[0]) ? $"_{sanitizedNs}" : sanitizedNs; // Handle namespace starting with digit
    }
}
